...
if(realCar):
    ...
    file_path = '/home/nvidia/romeoNzt/wp-2025-02-14-12-49-57.csv'
else:
    lidarscan_topic = '/scan'
    loc_topic = '/odom'
    drive_topic = '/nav'
    file_path = '/home/romeo/rcws/logs/wp-2025-02-11-18-51-31.csv'
...
class Node(object):
    def __init__(self):
        ...
        self.bubbles = []   #On va decouper la grille en une union (non disjointe) de disque de diametre 2*self.neighboor_dist repartis sur une grille de maille neighboor_dist*neigboor_dist. On pourra ainsi optimiser la recherche des voisins puisqu'ils seront necessairement dans les boules contenant le noeud
        self.neighboor = []
...
class RRT(object):
    def __init__(self):
        ...
        rospy.Subscriber(lidarscan_topic, LaserScan, self.scan_callback, queue_size=1)  #il s'agit du callback qui va déclencher le calcul du chemin
        ...
        self.pose = Point()
        self.orientation = 0
        self.is_orientation_up_to_date = False
    ...
    def map_callback(self, map_msg):
        if self.have_launched_the_map:      #On ne veut charger la carte qu'une seule fois à l'initialisation
            return 
        ...
        #on va donc pouvoir construire les bules dans le rectangle defini par les bords de la carte (self.gauche, haut, droite, bas)
        ...
        #le upper corner est defini par haut et gauche 
    ...
    def pose_callback(self, pose_msg):
        ...
        #Comme on ne peut utiliser trop souvent euler_from_quaternion, on effectue le calcul dans le pose callback lorsque l'on en a besoin uniquement
        ''' 
        if time.time() - self.t > 0.001:                    #On ne peut pas appeler trop souvent, euler_from_quaternion
            self.t = time.time()
            ...
            self.is_orientation_up_to_date = True
        '''
    
    def add_obstacle(self, cell):           #on ajoute un obstacle d'un rayon fixe a la carte locale
        grid = self.local_grid.data
        w = self.occupancy_grid.info.width
        h = len(grid) // w

        thickness = int(car_size // self.occupancy_grid.info.resolution)

        ligne, colonne = divmod(cell, w)
    ...
    def scan_callback(self, data):
        ...
        if self.have_launched_the_map:
            if not self.is_runing:              #on ne veut pas que le robot calcule un nouveau chemin tant qu'il n'a pas fini le precedent
                self.nb_iterations += 1
                #prendre en compte les donnees du lidar pour construire une carte local qui s ajoute a la carte generale
                angle = data.angle_min
                
                
                quaternion = np.array([self.pose_msg.pose.pose.orientation.x, self.pose_msg.pose.pose.orientation.y, self.pose_msg.pose.pose.orientation.z,self.pose_msg.pose.pose.orientation.w]) 
                angles = euler_from_quaternion(quaternion)
                self.orientation = angles[2]
                #takes perhaps to much time, esay improve : save the added points in a list and remove them one by one
                if self.is_orientation_up_to_date:
    ...
    def next_waypoint(self):
        '''Modifie le point cible si le robot est suffisamment proche du point cible actuel'''
        ...
        if(d < self.L*8):
            ...
            self.next_waypoint()                                        #on verifie que le nouveau point n'est pas trop proche du robot aussi
        
    
    def erase_local_grid(self):                         #on efface les cases de la grille mais on se concentre sur celle où il y a le circuit uniquement 
        for i in range(self.gauche, self.droite):
            for j in range(self.haut, self.bas):
                grid_index = i * self.occupancy_grid.info.width + j
                self.local_grid.data[grid_index] = 0
    

    def find_shortest_path(self, start_point, end_point, START_ID):
        '''Méthode principale qui va calculer le chemin le plus court entre deux points en utilisant l'algorithme RRT*'''
        ...
        #On initialise l'arbre avec le point de depart
        origin_node = Node()
        ...
        #on doit ajouter le noeud a la liste des bubbles de la grille qui contiennet bien le point
        temps = time.time()
        ...
        while(t < ITERATION):
            ...
            if t < len(self.curent_path):                               #on va essayer de suivre le chemin deja calcule si on en a un
                new_node = self.curent_path[t]
            else:
                if(t == ITERATION-1):                                   #on termine toujours par le but pour recalculer si un meilleur chemin a ete trouver depuis la derniere fois
            ...
            if(self.check_collision(nearest_node, new_node)):
                #on doit ajouter le noeud a la liste des bubbles de la grille qui contiennent bien le point
                ...
                #il faut maintenant relier le nouveau noeud au meilleur voisin puis lancer la procedure de rewiring
                ...
                rospy.loginfo_throttle(0.5,"le temps moyen des fonctions rewiring sur une iteration est de : " + str(self.total_time_rewiring / self.nb_iterations))
           

            #on verifie si le nouveau noeud est suffisamment proche du but
            if(self.is_goal(tree[-1], goal_x, goal_y)):
                ...
                if(distance < best_path_length):
                    ...
                    TEMPERATURE = 0.999                     #il n'est plus nécessaire de prendre le but comme sample point
    ...
    def find_map_corner(self):
        ...
        map_offset = 20
        #Les +map_offset sont la pour que les coins de la carte ne soient pas jamais explores 
    ...
    def sample(self, temperature, goal_x, goal_y):
        ...
        while(True):                    #on va chercher un point aleatoire dans la carte, on s'assure qu'il est dans la carte et qu'il n'est pas dans un obstacle
    ...
    def nearest(self, tree, sampled_point):
        ...
        #on va chercher dans les boules proche du noeud s'il y a d'autres points jusqu'a en trouver, (s'il y a beaucoup de point la recherche sera donc tres rapide)
        ...
        #On va parcourir les bulles de façon concentrique autour du sampled_point jusqu a trouver une boulle non vide
        for r in range(h + w):
    ...
    def check_collision(self, nearest_node, new_node):
        ...
        for x in range(dx + 1):
            ...
            if(self.occupancy_grid.data[index] + self.local_grid.data[index]> 60):     #on prend en compte la carte locale, evitement d'obstacle
            #if(self.occupancy_grid.data[index] > 60):                                   #on ne prend que la carte globale, on ne veut pas que le robot se prenne dans les murs
                return False
            if D >= 0:
                y += 1
                D -= 2*dx
    ...
    def near(self, tree, node):
        ...
        for buble in node.bubbles:
            for neighboor in buble:
                ...
                if d < 0.0001:              #on evite de considerer le noeud lui meme
                    continue
